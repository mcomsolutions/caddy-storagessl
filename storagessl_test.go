...
func TestNats_Lock(t *testing.T) {
	A := new(StorageParam)
	A.Host = "127.0.0.1"
	A.Port = "9582"
	A.DataBaseName = "mcom_adnud_dev"
	...
	err := A.Lock(ctx, "victor")
	if err != nil {
		fmt.Println("No puede crear el primer lock. Es incorrecto " + err.Error())
	} else {
		fmt.Println("Si puede crear el primer lock. Es correcto ")
	}

	err = A.Lock(ctx, "victor")
	if err != nil {
		fmt.Println("No puede crear el segundo lock. Es correcto " + err.Error())
	} else {
		fmt.Println("Si puede crear el segundo lock. Es incorrecto")
	}
	time.Sleep(15 * time.Second)
}

func TestNats_Lock2(t *testing.T) {
	A := new(StorageParam)
	A.Host = "127.0.0.1"
	A.Port = "9582"
	A.DataBaseName = "mcom_adnud_dev"
	...
	fmt.Println("Estoy probando el lock 2")
